          ...
          <ErrorBoundary key={location.pathname} title="مشکلی در نمایش این صفحه پیش آمد">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                /* `lg:h-full` زنجیرهٔ ارتفاع را کامل می‌کند تا صفحه‌ای که
                   می‌خواهد *دقیقاً* به قدِ ناحیهٔ محتوا باشد (مثل همکار در
                   صفحهٔ کامل) بتواند `h-full` بگیرد. بی این حلقهٔ گم‌شده،
                   چنین صفحه‌ای مجبور بود ارتفاعش را از `100vh` منهای یک
                   عددِ حدسی حساب کند — و هر تغییری در نوار بالا یا
                   فاصله‌های پوسته، بی‌صدا از اندازه خارجش می‌کرد.

                   فقط از `lg`: پایین‌تر از آن، خودِ `main` ارتفاعِ معین
                   ندارد و `h-full` به `auto` می‌رسد؛ یعنی این خط آن‌جا
                   بی‌اثر است و نه خطرناک. */
                className="lg:h-full"
              >
                <Outlet />
              </motion.div>
