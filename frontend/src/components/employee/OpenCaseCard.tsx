...
function SelfAssessmentForm({
  ...
  async function submit() {
    ...
    // پشیمانی روی کاری که برنمی‌گردد را کم می‌کند.
    //
    // متن پیش از این می‌گفت «برای ارزیاب ارسال می‌شود … در گفت‌وگو با ارزیاب
    // مطرح کنید» — که دیگر درست نیست و مهم‌تر، *برعکسِ* پیامِ موفقیتی بود که
    // یک ثانیه بعد نشان داده می‌شد («فقط خودتان و منابع انسانی»). قاعده در
    // `services/self_assessment.VIEWER_ROLES` است: مدیر مستقیم، معاونت و
    // مدیرعامل هیچ‌وقت نمی‌بینند. جملهٔ غلط، در لحظه‌ای که فرد دارد تصمیم
    // می‌گیرد چه بنویسد، بدترین جای ممکن برای غلط‌بودن است.
    const ok = await confirm({
      title: "خودارزیابی ثبت شود؟",
      danger: true,
      description:
        "دیدگاه شما پس از ثبت قابل ویرایش نیست. آن را فقط خودتان و منابع انسانی می‌بینید — مسئول مستقیم، معاونت و مدیرعامل به آن دسترسی ندارند.",
