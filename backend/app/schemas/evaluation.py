...
class EvaluationRead(BaseModel):
    ...
    #:
    #: به رابط می‌رسد چون نوارِ مراحل بی آن دروغ می‌گوید: مرحلهٔ منابع انسانی را
    #: «انجام‌شده» نشان می‌داد، در حالی که هیچ‌کس آن را انجام نداده. مثل
    #: `single_decider`، حالتی مجاز است که باید *دیده* شود، نه پنهان بماند.
    hr_review_skipped: bool = False
...
class MyOpenEvaluation(BaseModel):
    ...
    #: پروندهٔ خودِ کارمندانِ منابع انسانی مرحلهٔ منابع انسانی ندارد. این‌جا هم
    #: می‌آید چون نوارِ مراحلِ همین کارت، بی آن، مرحله‌ای را «انجام‌شده» نشان
    #: می‌دهد که هیچ‌کس انجامش نداده — و مخاطبِ این کارت خودِ همان فرد است.
    hr_review_skipped: bool = False
    #: این پرونده مرحلهٔ معاونتِ *جدا* ندارد — همان استدلالِ بالا، برای صندلیِ
    #: دیگر. دو شکل به این‌جا می‌رسند: زنجیرهٔ بی‌معاونت (صندلی خالی است، پس
    #: پرونده روی `hr_approved` روی میزِ مدیرعامل می‌نشیند) و مسیرِ «مدیر»
    #: (معاونت خودش نمره داده، پس آن مرحله مصرف شده).
    #:
    #: *شکلِ* زنجیره است و نه *هویتِ* صندلی‌ها — همان مرزی که این نما دارد.
    deputy_skipped: bool = False
