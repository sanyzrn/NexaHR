...
class Settings(BaseSettings):
    ...
    # سقفِ روزانهٔ پیامِ دستیار وقتی حسابِ کاربر عددِ خودش را ندارد. هر پیام یک
    # فراخوانِ پرداختی به سرویسِ مدل است و کلِ تاریخچهٔ گفت‌وگو را با خود می‌برد،
    # پس «بی‌حد» پیش‌فرضِ درستی نبود. صفر روی خودِ حساب یعنی «همین عدد»، و
    # ۱- یعنی بی‌حدِ صریح (`ai.resolve_daily_limit`).
    ai_daily_message_limit: int = 100

    # سقفِ نرخِ per-IP روی گفت‌وگوی دستیار. برای گفت‌وگوی واقعیِ آدم فراوان است و
    # برای یک حلقه بی‌فایده — و حلقه لازم نیست عمدی باشد: فرانت‌اند روی خطا
    # دوباره تلاش می‌کند.
    ai_chat_rate_limit: str = "20/minute"

    # منطقهٔ زمانیِ سازمان. ذخیره همیشه UTC است؛ این تنظیم فقط دو مرز را جابه‌جا
    # می‌کند: «امروز» در سنجشِ مهلت‌ها، و ساعتِ چاپ‌شده روی سندِ رسمی. توضیح
    # کاملش در `core/clock.py`.
    org_timezone: str = "Asia/Tehran"
    ...
    #: فاصلهٔ راستی‌آزماییِ *کاملِ* زنجیرهٔ ممیزی.
    #:
    #: زمان‌بند هر پنج دقیقه اجرا می‌شود و بررسیِ کامل از ابتدای تاریخ حساب
    #: می‌کند؛ هر پنج دقیقه زدنش یعنی همان هزینه‌ای که این فاز برای حذفش آمده،
    #: فقط از مسیرِ درخواست به مسیرِ پس‌زمینه منتقل شده باشد. روزی یک‌بار کافی
    #: است: تنها چیزی که این بررسی پیدا می‌کند، دست‌بردن مستقیم در دیتابیس است.
    audit_full_verify_interval_hours: int = 24
