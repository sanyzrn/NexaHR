...
#: نویسه‌های نامرئی که در یک سلولِ اکسل هیچ کاری ندارند و فقط از چسباندنِ متن
#: از Word یا وب می‌آیند: فاصلهٔ صفرعرض، اتصال‌دهندهٔ صفرعرض، BOM، نشانه‌های
#: جهت، و خط‌تیرهٔ نرم.
#:
#: نیم‌فاصله (`\u200c`) عمداً *نیست*: در املای فارسی یک نویسهٔ معنادار است و
#: «علی‌رضا» با «علیرضا» یکی نمی‌شود. حذفش یعنی بازنویسیِ نامِ آدم‌ها.
...
def _text(value: object) -> str:
    """متنِ یک سلول، پاک‌شده از نویسه‌های نامرئی.

    خطِ قبلی `value.replace("\u200c", "\u200c")` بود — هر دو طرف *همان*
    نیم‌فاصله. یعنی جایگزینی‌ای که کامنتش وعدهٔ «پاک‌کردنِ فاصله‌های نامرئی»
    می‌داد، هیچ‌وقت هیچ نویسه‌ای را عوض نمی‌کرد و تنها کارِ واقعیِ این خط
    تبدیلِ `\xa0` بود.

    آنچه واقعاً می‌آزارد، *بقیهٔ* نامرئی‌هاست: متنی که از Word یا یک صفحهٔ وب
    در سلول چسبانده می‌شود می‌تواند BOM یا نشانهٔ جهت داشته باشد، و آن نام
    در نگاه با نامِ درست یکی است ولی در مقایسهٔ رشته‌ای نه — و بدتر، همان
    نویسهٔ نامرئی در سندِ رسمی و کارنامه هم می‌نشیند.

    نیم‌فاصله دست‌نخورده می‌ماند؛ حذفش نامِ آدم را عوض می‌کند و نه اصلاحش.
    """
    if value is None:
        return ""
    if isinstance(value, str):
        return value.translate(_INVISIBLE).replace("\xa0", " ").strip()
