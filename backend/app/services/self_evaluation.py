...
def ensure_may_administer(record, current_user) -> None:
    """اقدامِ *اداری* روی یک پروندهٔ باز: لغو، جایگزینیِ مسئولِ مرحله، تمدیدِ مهلت.

    این سه، ابزارِ نجاتِ پروندهٔ گیرکرده‌اند و تا امروز فقط منابع انسانی به آن‌ها
    دسترسی داشت. برای پروندهٔ خودِ واحدِ HR — همانی که `hr_panel_is_shielded`
    می‌بنددش — نتیجه یک بن‌بستِ کامل بود:

    مسئولِ واحدِ HR از سازمان خارج می‌شود. پروندهٔ باز روی صندلیِ او می‌ماند.
    منابع انسانی نمی‌تواند ببیند، جایگزین بگذارد، یا لغو کند (سپر، ۴۰۳)؛ صاحبِ
    رفتهٔ صندلی نمی‌تواند وارد شود (۴۰۱)؛ و ایندکسِ یکتای جزئی اجازهٔ پروندهٔ
    تازه برای همان پرسنل را نمی‌دهد (۴۰۰). آن فرد *برای همیشه* غیرقابل‌ارزیابی
    می‌شد. با اجرا سنجیده شد؛ هر شش مسیر بسته بود.

    خودِ پیامِ سپر از قبل جوابِ درست را می‌گفت — «رسیدگی به آن با معاونت و
    مدیرعامل است» — ولی هیچ‌کدام از آن سه endpoint آن دو نقش را راه نمی‌داد.
    این تابع همان جمله را اجرایی می‌کند و نه یک ذره بیشتر:

    * منابع انسانی: دقیقاً همان گاردِ همیشگی (`ensure_hr_may_handle`).
    * مدیرعامل: روی پروندهٔ سپرشده مجاز است. بالای سرش کسی نیست که سپر از او
      محافظت کند، و اگر جانشینِ یک مدیرعاملِ رفته باشد، تنها کسی است که مانده.
    * معاونت: فقط روی پروندهٔ سپرشده‌ای که *خودش* در زنجیره‌اش نشسته. وگرنه
      این تابع به هر معاونتی حقِ لغوِ هر پروندهٔ سازمان را می‌داد.

    روی پروندهٔ غیرِ سپرشده هیچ چیز عوض نمی‌شود: کار با منابع انسانی است، و
    معاونت و مدیرعامل همان ۴۰۳ همیشگی را می‌گیرند.
    """
    ...
    administrative = HTTPException(
        status_code=http_status.HTTP_403_FORBIDDEN,
        detail=(
            "رسیدگیِ اداری به این پرونده (لغو، تغییر مسئول مرحله، تمدید مهلت) "
            "با منابع انسانی است."
        ),
    )
    # پیش‌فرض «رد» است و نه «عبور»: روترهای امروزی فقط سه نقش را به این‌جا
    # می‌رسانند، ولی گاردی که برای نقشِ ناشناخته بی‌صدا باز شود، همان اشکالی
    # است که این تابع دارد رفعش می‌کند — منتها یک لایه بالاتر.
    if current_user.role not in (UserRole.deputy, UserRole.ceo):
        raise administrative
    if not hr_panel_is_shielded(record):
        raise administrative
    if current_user.role is UserRole.ceo or current_user.id == record.deputy_user_id:
        return
    raise HTTPException(
        status_code=http_status.HTTP_403_FORBIDDEN,
        detail="شما در زنجیرهٔ این پرونده نیستید.",
    )
