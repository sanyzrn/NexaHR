...
CAPABILITY_LABELS: dict[Capability, str] = {
    ...
    Capability.manage_ai: "تنظیمات دستیار هوشمند",
}
...
def set_capabilities(
    ...
    # کسی نمی‌تواند مجوزی را که ندارد به *خودش* بدهد.
    #
    # تا امروز هیچ قاعده‌ای برای این حالت نوشته نشده بود — نه اجازه، نه منع؛
    # یعنی دارندهٔ `manage_capabilities` با یک کلیک هر مجوزِ دیگری را برای
    # خودش روشن می‌کرد و در لاگ فقط یک سطر می‌ماند: «فلانی به فلانی داد».
    #
    # این همان قاعده‌ای است که سامانه جای دیگر هم دارد
    # (`ensure_not_deciding_about_oneself`): کسی که موضوعِ یک تصمیم است، در
    # آن تصمیم نقش ندارد.
    #
    # **این گارد چه چیزی را نمی‌گیرد** کسی که واقعاً بخواهد، می‌تواند حسابِ
    # دومی بسازد، مجوز را به آن بدهد و با آن وارد شود. جلوی آن را نمی‌شود با
    # یک شرط گرفت. چیزی که این گارد می‌دهد، *ردِ دو نفره* است: ارتقای مجوز از
    # این پس دستِ کم دو حساب و دو ردیفِ لاگ لازم دارد، و همان تفاوت است که
    # در بازبینی دیده می‌شود.
    #
    # *گرفتنِ* مجوز از خود آزاد است — گاردِ «آخرین دارنده» پایین‌تر تنها
    # حالتِ خطرناکش را می‌بندد. کسی که اختیارِ خودش را کم می‌کند، مسئله‌ای
    # نیست.
    #
    # و ترتیبِ این دو گارد عمدی است: باری که هم‌زمان می‌گیرد و می‌دهد هر دو
    # را روشن می‌کند، و این یکی جلوتر می‌آید چون پرسشِ *اختیار* است («این
    # تصمیم اصلاً با توست؟») و آن یکی پرسشِ اعتبارِ محتوا — همان ترتیبی که
    # `require_capability` هم دارد.
    if user_id == current_user.id and (desired - current):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "نمی‌توانید مجوزی را به حساب خودتان اضافه کنید؛ از یک مدیر سامانهٔ "
                "دیگر بخواهید این کار را انجام دهد. (گرفتنِ مجوز از خودتان آزاد است.)"
            ),
        )
