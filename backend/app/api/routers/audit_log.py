...
def audit_integrity(
    full: bool = Query(
        default=False,
        description="از ابتدای زنجیره بسنج، نه از لنگر. گران است و عمداً درخواستی.",
    ),
    db: Session = Depends(get_db),
    ...
    یک لاگ حسابرسی فقط وقتی «مدرک» است که بشود نشان داد دست‌کاری نشده؛ بدون این
    endpoint، خودِ زنجیره هم چیزی جز چند ستون بی‌استفاده نبود.

    **پیش‌فرض از لنگر به بعد است، نه از ابتدا.** بررسیِ کامل از ابتدای تاریخ
    حساب می‌کند و آن عدد هرگز کوچک نمی‌شود؛ جایش جاروی پس‌زمینه است
    (`scheduled.run_audit_anchor_sweep`) که روزی یک‌بار کامل می‌سنجد و لنگر را
    جلو می‌برد. این‌جا فقط از لنگر تا سرِ زنجیره سنجیده می‌شود — یعنی هزینه به
    *فعالیتِ از دیشب تا حالا* بند است.

    و همان لنگر هم وارسی می‌شود: اگر هشی که نگه داشته با خودِ لاگ نخوانَد،
    پاسخ قرمز است. بی آن، لنگر راهی برای دور زدنِ کلِ بررسی می‌شد.

    `full=true` همان بررسیِ قدیمی است، برای وقتی که کسی صریحاً بخواهد کلِ تاریخ
    را از نو بسنجد. پاسخ در هر دو حالت `full` و `anchor_verified_at` را دارد،
    تا «سبز» با «سبزِ کامل» اشتباه گرفته نشود.
    ...
    """
    return AuditIntegrityRead(**verify_chain(db, since_anchor=not full))
