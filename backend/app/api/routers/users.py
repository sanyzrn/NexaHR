...
def create_user(
    ...
    user = User(
        ...
        is_active=True,
        # رمزی که *کسِ دیگری* انتخاب کرده، موقت است.
        #
        # مسیر اکسل از ابتدا این را می‌گذاشت و استدلالش هم آن‌جا نوشته شده
        # (`personnel_import.py:69`): «رمزی که در یک فایلِ اکسلِ دست‌به‌دست‌شده
        # نوشته شده نباید در استفاده بماند». رمزی که منابع انسانی در یک فرمِ وب
        # تایپ می‌کند و بعد با تلفن یا پیام می‌فرستد، دقیقاً همان ردهٔ خطر است —
        # و بازنشانیِ رمز در همین فایل (`update_user`) هم می‌گذاردش. فقط ساختِ
        # حساب جا افتاده بود.
        #
        # ماشینِ اجبار از قبل کامل است: گاردِ سرور یک allowlist است
        # (`api/deps.py:17-22`)، پس این یک خط تمامِ کار است.
