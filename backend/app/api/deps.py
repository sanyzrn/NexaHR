...
_FORCED_PASSWORD_CHANGE_EXEMPT_PATHS = frozenset(
    {
        ...
        "/api/auth/me",
        # مجوزهای *خودِ* فراخوان. چیزی جز آنچه او همین حالا دارد لو نمی‌دهد،
        # و رابط بی آن نمی‌تواند کاربر را درست مسیریابی کند.
        #
        # بی این معافیت، بلافاصله پس از تغییر اجباریِ رمز یک دروغِ تمام‌عیار
        # دیده می‌شد: درخواست ۴۰۳ می‌گرفت، `PermissionsContext` روی خطا همه‌چیز
        # را «ندارد» می‌خواند، و صفحهٔ مدیریت به حسابی که *همهٔ* اختیارات را
        # دارد می‌گفت «شما مجوز مدیریت سامانه را ندارید». اولین صفحه‌ای که
        # مدیرِ سامانه پس از تغییر رمزش می‌بیند، همین است — و تنها درمانش یک
        # رفرشِ دستی بود.
        #
        # از وقتی `moduleEnabled` هم fail-closed شد، همان خطا لینک‌های
        # ماژول‌دارِ منو را هم می‌برد. fail-closed درست است؛ چیزی که غلط بود
        # این ۴۰۳ است.
        "/api/administration/my-permissions",
    }
)
