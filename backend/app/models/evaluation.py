...
class EvaluationRecord(Base):
    ...
    #: این پرونده مرحلهٔ بررسیِ منابع انسانی *ندارد* — چون موضوعش خودش منابع
    #: انسانی است.
    #:
    #: مرحلهٔ HR تنها مرحله‌ای است که صاحبِ از پیش تعیین‌شده ندارد و از یک صفِ
    #: مشترک برداشته می‌شود؛ آن صف برای *همهٔ* کاربران HR دیده می‌شود. پس
    #: پروندهٔ یک کارمندِ منابع انسانی، در همان صفی می‌نشیند که خودش عضوش است:
    #: نمره‌اش را پیش از ابلاغ می‌دید، و در تیمِ کوچکِ HR داورش یا خودش بود یا
    #: مسئولِ مستقیمش که همان نمره را داده بود.
    #:
    #: قرینهٔ `unit_supervisor_user_id IS NULL` (مسیر «مدیر») و
    #: `deputy_user_id IS NULL` است: مرحله‌ای که داورِ بی‌طرف ندارد، نباید
    #: پرونده را نگه دارد. تفاوتش این است که مرحلهٔ HR صندلیِ از پیش پرشده
    #: ندارد، پس نبودنش را نمی‌شود با NULL گفت و ستونِ خودش را لازم دارد.
    #:
    #: در لحظهٔ ساخت مهر می‌شود و نه در لحظهٔ خواندن — مثل بقیهٔ زنجیره. اگر
    #: زنده محاسبه می‌شد، عوض‌شدنِ نقشِ یک نفر وسط چرخه شکلِ پروندهٔ در جریان را
    #: از زیر پای تأییدکننده‌اش عوض می‌کرد و پروندهٔ نشسته در صفِ HR بی‌صدا
    #: غیرقابل‌تأیید می‌شد.
    hr_review_skipped: Mapped[bool] = mapped_column(
