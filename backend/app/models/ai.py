...
class AiSettings(Base):
    ...
    #: آیین‌نامهٔ عملکردِ *همین* سازمان — متنی که مدیر وارد می‌کند.
    #:
    #: چرا این‌جا و نه بازیابیِ برداری (RAG): در این مقیاس — یک سازمان، چند
    #: ده شاخص، آیین‌نامه‌ای چندصفحه‌ای — pgvector یک مایگریشن، یک خطِ لولهٔ
    #: embedding و حسابداریِ تازگی اضافه می‌کند تا به پرسشی جواب بدهد که یک
    #: بخشِ چندکیلوبایتیِ پرامپت قطعی‌تر جواب می‌دهد. اگر روزی اسناد از حدودِ
    #: بیست صفحه گذشت یا هفتگی عوض شد، همان‌جا تجدیدنظر می‌شود.
    #:
    #: و چرا جدا از `instructions`: آن متن می‌گوید «چطور جواب بده» و این
    #: می‌گوید «قاعدهٔ سازمان چیست». اولی دستور است و دومی *سند* — و در
    #: پرامپت هم دقیقاً با همین تفاوت جا می‌گیرند.
